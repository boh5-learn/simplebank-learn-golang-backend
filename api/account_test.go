...
func TestGetAccountAPI(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			accountID: account.ID,
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, api.AuthorizationTypeBearer, "unauthorizedUser", time.Minute)
			},
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
					GetAccount(gomock.Any(), gomock.Eq(account.ID)).
...
func TestDeleteAccountAPI(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:      "InternalError DeleteAccount",
			accountID: account.ID,
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, api.AuthorizationTypeBearer, user.Username, time.Minute)
			},
